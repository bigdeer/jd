 ...
 * [env]
 * export XM_SPORT_TOKEN=YOUR_TOKEN
 */
...
async function start() {
    ...
    if (!login_token && user) {
        let { login_token1 = 0, userid } = await login(user, password)
        if (login_token1 === 0) {
            console.log('登陆失败！');
            return "login fail!"
        }
        login_token = login_token1;
    }
    if (login_token) {
...
async function login(user, password) {
    ...
    const login_token1 = res2.data.token_info.login_token;
...
if ($.isNode()) {
    user = process.env.MI_USERNAME
    password = process.env.MI_PASSWORT
